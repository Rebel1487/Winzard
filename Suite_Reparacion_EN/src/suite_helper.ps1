...
function Get-SysInfo {
    ...
    "CPU=$cpuName"
    "RAM=$ramGB GB"
    "DISK=C: $freeGB GB free of $totGB GB"
...
function Get-HealthScore {
    ...
    if ($sym['smartBad']) { $reasons += "Disk with degraded SMART (-25)" }
    # Espacio libre
    $c = Get-PSDrive C; $freeGB = [math]::Round($c.Free/1GB,1)
    $sym['freeGB'] = $freeGB
    if     ($freeGB -lt 5)  { $reasons += "Less than 5 GB free on C: (-15)" }
    elseif ($freeGB -lt 15) { $reasons += "Low free space on C: (-6)" }
    # Reinicio pendiente
    ...
    if ($pend) { $reasons += "Pending reboot (-5)" }
    ...
    if ($bsod -gt 0) { $reasons += "Recent crashes/BSOD: $bsod (-18)" }
    if ($disk -gt 0) { $reasons += "Recent disk/NTFS errors: $disk (-12)" }
    if ($whea -gt 0) { $reasons += "Hardware errors (WHEA): $whea (-12)" }
    if ($crit.Count -gt 25) { $reasons += "Many critical events in 48h: $($crit.Count) (-6)" }
    # Servicios clave parados
    $svcStopped = 0
    foreach ($svc in 'wuauserv','BITS','Winmgmt','EventLog') {
        $s = Get-Service $svc -ErrorAction SilentlyContinue
        if ($s -and $s.Status -ne 'Running' -and $s.StartType -ne 'Disabled') { $svcStopped++; $reasons += "Service $svc stopped (-4)" }
    }
    $sym['svcStopped'] = $svcStopped
    # Devices con problema
    $prob = @(Get-CimInstance Win32_PnPEntity | Where-Object { $_.ConfigManagerErrorCode -gt 0 }).Count
    $sym['devProblems'] = $prob
    if ($prob -gt 0) { $reasons += "Devices with errors: $prob" }
    ...
    if (($st.PSObject.Properties.Name -contains 'diag') -and $st.diag) {
        if ($st.diag.ram -and ([string]$st.diag.ram.status -eq 'suspect')) {
            $sym['ramSuspect'] = $true; $reasons += "RAM suspicious (-10)"
        }
        if ($st.diag.battery -and $st.diag.battery.present) {
            ...
            if ($null -ne $bpRaw -and [string]$bpRaw -ne '') {
                ...
                if ($null -ne $bp) {
                    ...
                    if ($bp -ge 0 -and $bp -lt 50) { $reasons += "Battery heavily degraded: $bp% (-8)" }
                }
            }
        }
        if ($st.diag.network -and (($st.diag.network.connected -eq $false) -or ($st.diag.network.dns_ok -eq $false))) {
            $sym['netProblem'] = $true; $reasons += "Persistent network problems (-8)"
        }
    }
...
function Get-Forensics {
    ...
    if ($out.Count -eq 0) { "OK|0|-|No critical errors in the last 7 days." } else { $out }
}
...
function Get-DeviceProblems {
    ...
    if ($p.Count -eq 0) { "OK|No devices with problems."; return }
...
function New-HtmlReport($outPath) {
    ...
    try {
        ...
        $bandLabel = { param($s) if ($s -eq '-' -or $null -eq $s -or [string]$s -eq '') { 'no data' } else { $v=0; try { $v=[int]$s } catch { return 'no data' }; if ($v -ge 80) {'Good'} elseif ($v -ge 50) {'Fair'} else {'Critical'} } }
        $offsetOf = { param($s) $v=0; try { $v=[int]$s } catch { $v=0 }; if ($v -lt 0){$v=0}; if ($v -gt 100){$v=100}; [math]::Round($circ * (1 - ($v/100.0)), 2) }
        $statusIcon = {
            param($res)
            switch ([string]$res) {
                'OK'    { "<svg viewBox='0 0 24 24' class='svgico' role='img' aria-label='successful'><circle cx='12' cy='12' r='11' fill='#22c55e'/><path d='M7 12.4l3.2 3.2L17 8.8' fill='none' stroke='#04210f' stroke-width='2.6' stroke-linecap='round' stroke-linejoin='round'/></svg>" }
                'WARN'  { "<svg viewBox='0 0 24 24' class='svgico' role='img' aria-label='warning'><path d='M12 2.5L23 21.5H1z' fill='#f59e0b'/><rect x='11' y='8.5' width='2' height='7' rx='1' fill='#3a2400'/><circle cx='12' cy='18' r='1.3' fill='#3a2400'/></svg>" }
                ...
                'SKIP'  { "<svg viewBox='0 0 24 24' class='svgico' role='img' aria-label='skipped'><circle cx='12' cy='12' r='11' fill='#64748b'/><rect x='6.5' y='11' width='11' height='2' rx='1' fill='#0b1220'/></svg>" }
        ...
        $delta = 0; $deltaTxt = 'no comparison'
        if ($hasBoth) { $delta = [int]$st.score_after - [int]$st.score_before; $sign = if ($delta -ge 0) {'+'} else {''}; $deltaTxt = "$sign$delta points" }
        ...
        if ($history -and $history.Count -gt 0) {
            $historyHtml += "<div class='trend-title'>Health History (Last runs)</div><div class='trend-list'>"
        ...
        $sysOrder = @(@('OS','Operating System'),@('SYSTEM','System Model'),@('CPU','Processor'),@('RAM','RAM Memory'),@('DISK','Disk C:'),@('UPTIME','Uptime'),@('USER','User'))
        $sysCards = ''
        ...
        $machine = $sysMap['SYSTEM']; if (-not $machine) { $machine = $env:COMPUTERNAME }
        ...
        foreach ($f in $findings) {
            $txt = [string]$f
            $sev = 'info'; $sevTxt = 'Notice'
            if ($txt -match '(?i)SMART|BSOD|crash|WHEA|hardware|unrepairable|damaged|repository|integrity') { $sev='high'; $sevTxt='Important' }
            elseif ($txt -match '(?i)space|pending reboot|network|battery|driver|device|\bRAM\b|service') { $sev='med'; $sevTxt='Review' }
            ...
            if ($txt -match '(?i)SMART')          { $stepsList.Add('Back up your data as soon as possible: a disk with degraded SMART can fail. Consider replacing it.') }
            elseif ($txt -match '(?i)space')    { $stepsList.Add('Free up space on C: (uninstall what you don''t use or use Storage Sense). Aim for more than 15 GB free.') }
            elseif ($txt -match '(?i)\bRAM\b|memory') { $stepsList.Add('Run Windows Memory Diagnostic (mdsched.exe) and reboot to check the RAM.') }
            elseif ($txt -match '(?i)battery')    { $stepsList.Add('The battery is degraded. Check the battery report (powercfg /batteryreport) and consider replacing it.') }
            elseif ($txt -match '(?i)pending reboot') { $stepsList.Add('Reboot the PC to apply pending changes before continuing repairs.') }
            elseif ($txt -match '(?i)unrepairable|repository|integrity') { $stepsList.Add('Damaged components remain. Run DISM with a valid source (install.wim) and run SFC again.') }
            elseif ($txt -match '(?i)driver|device') { $stepsList.Add('Update the drivers of the failing devices from the maker''s site or Windows Update.') }
            elseif ($txt -match '(?i)network|DNS')        { $stepsList.Add('Check the network connection and DNS. If it persists, try a public DNS (1.1.1.1 / 8.8.8.8).') }
        }
        $noFind = ($findings.Count -eq 0)
        if ($noFind) { $findHtml = "<li class='find find-ok'><span class='sev sev-ok'>All OK</span><span class='find-txt'>No relevant problems were detected during diagnosis.</span></li>" }
        ...
        if ($cERR -gt 0) { $stepsHtml = "<li class='step-li'><span class='step-ic'>&#10148;</span><span>Some phases had errors: check the detailed log in the WPI_Suite\Logs folder.</span></li>" + $stepsHtml }
        if (-not $stepsHtml) { $stepsHtml = "<li class='step-li step-ok'><span class='step-ic'>&#10003;</span><span>No pending actions. Reboot the PC to make sure all changes are applied.</span></li>" }
        ...
        if (($st.PSObject.Properties.Name -contains 'diag') -and $st.diag) {
            ...
            if ($d.ram) {
                ...
                $rt = switch ($rs) { 'ok' {'No errors detected'} 'suspect' {'Suspect'} default {'Not evaluated'} }
                $mds = if ($d.ram.recommend_mdsched) { "<div class='d-hint'>Recommended: run Windows Memory Diagnostic (mdsched).</div>" } else { '' }
                $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-ram'></span>RAM Memory</div><div class='d-pill pill-$rp'>$rt</div>$mds</div>"
            }
            if ($d.battery) {
                if ($d.battery.present) {
                    ...
                    if ($null -ne $bpRaw -and [string]$bpRaw -ne '') {
                        ...
                        $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-bat'></span>Battery</div><div class='bat-bar'><span style='width:$bp%;background:$bpcol'></span></div><div class='d-sub'>Estimated health: <b style='color:$bpcol'>$bp%</b></div></div>"
                    } else {
                        $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-bat'></span>Battery</div><div class='d-pill pill-unknown'>Present, health unknown</div></div>"
                    }
                } else {
                    $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-bat'></span>Battery</div><div class='d-pill pill-unknown'>Not present (desktop PC)</div></div>"
                }
            }
            if ($d.network) {
                $cc = if ($d.network.connected) {'good'} else {'bad'}
                $ct = if ($d.network.connected) {'Connected'} else {'No connection'}
                $dc = if ($d.network.dns_ok) {'good'} else {'bad'}
                $dt = if ($d.network.dns_ok) {'DNS OK'} else {'DNS failing'}
                ...
                if (($d.network.PSObject.Properties.Name -contains 'dns_ms') -and $null -ne $d.network.dns_ms -and [string]$d.network.dns_ms -ne '') {
                    ...
                    $lat = "<div class='d-sub'>DNS latency: <b style='color:$lc2'>$ms ms</b></div>"
                }
                $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-net'></span>Network</div><div class='pill-row'><span class='d-pill pill-$cc'>$ct</span><span class='d-pill pill-$dc'>$dt</span></div><div class='d-sub'>$det</div>$lat</div>"
            }
            if (($d.PSObject.Properties.Name -contains 'smart') -and $d.smart -and $d.smart.available) {
                $sm = $d.smart
                $pf = if ($sm.predict_fail) { "<span class='d-pill pill-bad'>Predicts failure</span>" } else { "<span class='d-pill pill-good'>No alert</span>" }
                $extra = ''
                if ($null -ne $sm.temp_c -and [string]$sm.temp_c -ne '') { $tc=0; try{$tc=[int]$sm.temp_c}catch{}; $tcol = if ($tc -lt 50){'#22c55e'} elseif ($tc -lt 65){'#f59e0b'} else {'#ef4444'}; $extra += "<div class='d-sub'>Temperature: <b style='color:$tcol'>$tc &deg;C</b></div>" }
                if ($null -ne $sm.wear_pct -and [string]$sm.wear_pct -ne '') { $wp=0; try{$wp=[int]$sm.wear_pct}catch{}; $wcol = if ($wp -lt 50){'#22c55e'} elseif ($wp -lt 80){'#f59e0b'} else {'#ef4444'}; $extra += "<div class='d-sub'>Wear (SSD): <b style='color:$wcol'>$wp%</b></div>" }
                if ($null -ne $sm.poh -and [string]$sm.poh -ne '') { $extra += "<div class='d-sub'>Power-on hours: <b>$(& $enc $sm.poh)</b></div>" }
                $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-smart'></span>Disk health (SMART)</div><div class='pill-row'>$pf</div>$extra</div>"
            }
            if (($d.PSObject.Properties.Name -contains 'bcd') -and $d.bcd) {
                $bok = if ($d.bcd.ok) {'good'} else {'bad'}
                $btx = if ($d.bcd.ok) {'Boot configuration correct'} else {'Boot with issues'}
                $bdet = if ([string]$d.bcd.details -ne '') { "<div class='d-sub'>$(& $enc $d.bcd.details)</div>" } else { '' }
                $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-boot'></span>Boot (BCD)</div><div class='d-pill pill-$bok'>$btx</div>$bdet</div>"
            }
            if (($d.PSObject.Properties.Name -contains 'startup') -and $d.startup -and @($d.startup).Count -gt 0) {
                ...
                $diagCards += "<div class='dcard dcard-wide'><div class='d-h'><span class='d-ic ic-start'></span>Startup programs</div><ul class='dev-list'>$items</ul></div>"
            }
            if (($d.PSObject.Properties.Name -contains 'processes') -and $d.processes -and @($d.processes).Count -gt 0) {
                ...
                $diagCards += "<div class='dcard'><div class='d-h'><span class='d-ic ic-proc'></span>Processes using most memory</div><ul class='dev-list'>$items</ul></div>"
            }
            if ($d.devices -and @($d.devices).Count -gt 0) {
                $items = ''
                foreach ($dev in @($d.devices)) { $items += "<li>$(& $enc $dev.name) <span class='muted'>(code $(& $enc $dev.code))</span></li>" }
                $diagCards += "<div class='dcard dcard-wide'><div class='d-h'><span class='d-ic ic-dev'></span>Devices with warnings</div><ul class='dev-list'>$items</ul></div>"
            }
        }
        $diagSection = ''
        if ($diagCards) { $diagSection = "<h2 id='diag' class='sec-h'>Extended diagnosis</h2><div class='dgrid'>$diagCards</div>" }
...
<title>Repair Report - WPI Suite v3.1</title>
<style>
...
<div class='wrap'>
  <div class='topbar'>
    <div class='brand'>
      ...
      <div>
        <h1>Repair Report <span class='badge'>WPI SUITE v3.1</span></h1>
        <div class='sub'>$(& $enc $machine) &nbsp;&middot;&nbsp; generated on $now</div>
      </div>
    </div>
    <div class='btns'>
      <button class='toggle' onclick="window.print()">Print / PDF</button>
      <button class='toggle' id='copybtn' onclick="copyResumen()">Copy summary</button>
      <button class='toggle' id='themebtn' onclick="toggleTheme()">Light/Dark theme</button>
    </div>
  </div>

  <nav class='toc' aria-label='Index'>
    <a href='#resumen'>Summary</a>
    <a href='#fases'>Phases</a>
    <a href='#hallazgos'>Findings</a>
    <a href='#pasos'>Next steps</a>
  ...
  <div class='hero section'>
    <div class='card gaugewrap'>
      <svg viewBox='0 0 200 200' class='gauge' role='img' aria-label='Health score $mainScore out of 100'><circle class='track' cx='100' cy='100' r='84'/><circle class='fill' cx='100' cy='100' r='84' style='--circ:$circ;--target:$mainOffset;stroke:$mainColor'/><text x='100' y='112' class='g-num' style='fill:$mainColor'>$mainScore</text></svg>
      <div class='g-label' style='color:$mainColor'>Health: $mainLabel</div>
      <div class='g-cap'>SCORE OUT OF 100</div>
      $compareSection
      $historyHtml
    </div>
    <div class='hero-side'>
      <div class='card'>
        <div class='chips'>
          ...
          <div class='chip'><div class='n c-warn'>$cWARN</div><div class='l'>WARNINGS</div></div>
          <div class='chip'><div class='n c-err'>$cERR</div><div class='l'>ERRORS</div></div>
          <div class='chip'><div class='n c-skip'>$cSKIP</div><div class='l'>SKIPPED</div></div>
        </div>
      </div>
      <div class='card'>
  ...
  <div class='section'>
    <h2 id='fases' class='sec-h'>Phases timeline ($totalPh)</h2>
    <div class='timeline'>$rows</div>
    <div class='barchart'>$bars</div>
  </div>

  <div class='section'>
    <h2 id='hallazgos' class='sec-h'>Findings and root cause</h2>
    <ul class='finds'>$findHtml</ul>
  </div>

  <div class='section'>
    <h2 id='pasos' class='sec-h'>Recommended next steps</h2>
  ...
  <div class='foot'>
    WPI &middot; Emergency Repair Suite for Windows 10/11 &middot; read-only report.<br>
    Backups and logs are in the WPI_Suite folder next to the program.
  </div>
</div>
<script>
...
function fb(txt,ok){try{var a=document.createElement('textarea');a.value=txt;a.style.position='fixed';a.style.left='-9999px';document.body.appendChild(a);a.select();document.execCommand('copy');a.remove();ok();}catch(e){flash('Could not copy');}}
function copyResumen(){var p=[];var t=document.querySelector('h1');if(t)p.push(t.innerText.trim());var s=document.querySelector('.sub');if(s)p.push(s.innerText.trim());var ex=document.querySelector('.exec');if(ex)p.push('\n'+ex.innerText.replace(/\n{2,}/g,'\n').trim());var h=document.getElementById('hallazgos');if(h&&h.parentNode)p.push('\n'+h.parentNode.innerText.trim());var txt=p.join('\n');function ok(){flash('Summary copied');}if(navigator.clipboard&&navigator.clipboard.writeText){navigator.clipboard.writeText(txt).then(ok,function(){fb(txt,ok);});}else{fb(txt,ok);}}
</script>
</body>
</html>
"@
...
# --- Devices para diag (Req 15.3/15.4) -------------------------------
...
switch ($Action.ToLower()) {
    ...
    'diagfull' {
        ...
        if ($null -eq $dev) {
            $st.diag.devices = @()
            $devLine = "DEVICES_STATUS=info not available"
        } else {
            $st.diag.devices = @($dev)

...
> Paste this into the GitHub Release description for the `v1.0.0` tag.
> *(Texto de la pestaña Releases. En inglés para más alcance; abajo va un resumen en español.)*
...
You always see, at a glance, **what's left to do** — no guessing.

### 💿 Build your own custom Windows ISO (the star feature)
The expert-level section, now **guided step by step** so you never get lost: an **8-step wizard** with a **visual breadcrumb** (you always know where you are and what's left) and a green **"NOW:"** banner telling you exactly what to do. It integrates **offline**: debloat before install, **driver injection**, your **apps + tweaks** for first boot, **offline winget**, and a tailored **`autounattend.xml`** (local account, Win11 bypass…). Reassembles with **oscdimg**, includes an **ISO verifier** and **smart reminders** about the consequences of each choice (drivers baked-in vs. manual, the disk-wiping "VM mode", how to leave Rufus…).
...
📖 Full guide: [README (EN)](../../README_EN.md) · [Español](../../README.md)
...
### 🇪🇸 Resumen en español
**WPI Moderno v1.0.0** — centro de mando todo-en-uno para Windows 10/11: instala **+360 apps** con winget, **mantiene todo actualizado de verdad** (con verificación real de versión), **detecta lo que ya tienes** (apps instaladas, tweaks aplicados y bloatware, marcados por colores), optimiza con **40+ tweaks**, limpia bloatware, controla Windows Update, repara con una **Suite de 17 fases** y —la joya— **crea tu propia ISO de Windows a medida** con un asistente guiado. Bilingüe ES/EN, **3 temas**, sin telemetría, código abierto (MIT).
